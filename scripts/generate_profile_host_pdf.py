...
def footer(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, 0.30 * inch, "AUTOBOT Multi-Profile Host v0.13.2-beta.1")
...
def build_pdf():
    ...
    doc = SimpleDocTemplate(
        ...
        title="AUTOBOT v0.13.2-beta.1 Multi-Profile Host Setup",
    ...
    story.append(Paragraph("AUTOBOT v0.13.2-beta.1", title))
    ...
    item("Download and extract the v0.13.2 beta release ZIP. Do not run setup from inside the ZIP.")
    ...
    item("Double-click START-PROFILE-HOST-WINDOWS.cmd or START-PROFILE-HOST-MAC.command. It opens only missing Chrome profiles; no Terminal command is needed.")
    ...
    item("Keep all event windows visible for the 20-second local load simulation. It measures host event-loop delay, browser frame response, DOM scan cost, and free memory.")
    item("The recent recommendation is the automatic limit for selected workers on that host. Then click Select for operations.")
    ...
    callout("<b>Exact build required:</b> The host and every numbered AUTOBOT extension must report v0.13.2-beta.1 before the Command Center will activate a live run.")
    ...
    item("Leave experimental 15ms release lanes OFF unless repeated mock-event A/B tests show a benefit on that host.")
    item("From two seconds before release, nonessential polling, logging, and storage writes pause while the prepared execution capsule uses its cached ticket controls.")
